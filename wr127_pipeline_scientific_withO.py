...
# This version re-includes the HeI5876 O-star line for testing.
# It uses the conservative 20A half-width for EM lines to ensure robust WR fits.
...
# --- USER CONFIGURATION ---
#
# Path to directory holding pre-normalised FITS and optional HJD file
...
# Re-including the O-star line as requested for this test
O_ABS_LINES = {"HeI5876": 5875.625}
# ==============================================================================

# This will now contain both EM and ABS lines
...
def fit_gauss_windowed(wl, flux, rest_wave, is_emission=True,
                       # Using a more conservative 20A half-width
                       # to prevent fits from getting lost (as seen in NIV5785/95)
                       em_halfwidth=20.0, abs_halfwidth=15.0,
...
def bootstrap_rv_error(wl, flux, rest_wave, popt, nboot=200, rng=None, is_emission=True,
                       # Must match windows from main fit
                       em_halfwidth=20.0, abs_halfwidth=15.0):
...
def aggregate_results(results):
    ...
    safe_write(abs_df, ABS_SUM_CSV) # This will now contain the (bad) HeI5876 fit
    ...
    Ko_samps = draw_weighted_mean_k(abs_df) # This will now be the ~774 km/s samples
    summary = {}
...
def main():
    ...
    try:
        with open(PIPE_README, "w") as fh:
            ...
            fh.write("\nNOTE: This version includes the HeI5876 O-star line for testing.\n")
